class ChargeMaster
  ...
  def self.apply_discounts(invoice_id, discounts)
    ...
    if !discounts.nil?
      ...
      ChargeMaster.apply_largest_discount(invoice, ordered_discounts)
    else
      false
    end
  end
  ...
  def self.apply_largest_discount(invoice, discounts)
    if !discounts.empty?
